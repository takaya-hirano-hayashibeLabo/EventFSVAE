...
class EventFSVAE(nn.Module):
    ...
    def loss_func_van_rossum(self,input_events, reconst_events, q_z,p_z):
        """
        van rossum距離 + mmd距離
        
        :param *_events: (N,C,event_dim,event_dim,T)
        :param q_z is q(z|x): (N,latent_dim,k,T)
        :param p_z is p(z): (N,latent_dim,k,T)
        """
        
        #>> resonst loss >>
        input_r=double_exp_filter(event=input_events)
        reconst_r=double_exp_filter(event=reconst_events)
        recons_loss=(input_r-reconst_r)**2
        recons_loss=torch.sum(recons_loss,dim=-1) #時間方向に足す
        recons_loss=torch.sum(recons_loss,dim=0) #バッチ方向に足す
...
class EventFSVAELarge(EventFSVAE):
    def __init__(self):
        super(EventFSVAE, self).__init__()
        in_channels = glv.network_config['in_channels']
        latent_dim = glv.network_config['latent_dim']
        self.latent_dim = latent_dim
        self.n_steps = glv.network_config['n_steps']
