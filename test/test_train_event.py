...
def main():
    ...
    for _ in range(class_size):
        input_c=[]
        for _ in range(T):
            rand=np.random.uniform(0,1,size=(M,N))
            positive_idx=np.where(((xx)**2+(yy)**2)<(M/3)**2)
            negative_idx=np.where(((xx-M)**2+(yy-N)**2)<(M/3)**2)
            input_t=np.zeros(shape=(M,N))
            input_t[positive_idx]=np.where(rand[positive_idx]>0.5,1,0)
            input_t[negative_idx]=np.where(rand[negative_idx]>0.5,-1,0)
            input_c.append(input_t)
        input.append(input_c)
    ...
    result_path=f"{PARENT}/test_train_event"
    loss=pd.DataFrame([])
    if  args.is_train:
        ...
        torch.save(net.to(DEVICE).state_dict(), f'{result_path}/test_train_event.pth')
    else:
        net.load_state_dict(torch.load(f"{PARENT}/test_train_event/test_train_event.pth"))
    #>> 学習 >>
    ...
    net.eval()
    with torch.no_grad():
        net.load_state_dict(torch.load(f=f"{result_path}/test_train_event.pth",map_location=DEVICE))
    ...
    ani.save(f"{PARENT}/test_train_event/in_trained_event_view.gif")
    fig.clear()
    plt.close()
    #>> 描画 >>
